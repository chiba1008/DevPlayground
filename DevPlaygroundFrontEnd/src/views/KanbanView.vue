<template>
  <div class="kanban-container">
    <div class="kanban-header">
      <h1>カンバンボード</h1>
      <p>ドラッグ&ドロップでタスクのステータスを変更できます</p>
    ...
    <div class="kanban-board">
      <div v-for="status in statuses" :key="status.key" class="kanban-column">
        <div class="column-header" :class="`header-${status.key.toLowerCase().replace('_', '-')}`">
          <h3>{{ status.label }}</h3>
...
const handleDrop = async (event: DragEvent, newStatus: TodoStatus) => {
  ...
  try {
    ...
    // Update local state
    const todoIndex = todos.value.findIndex(t => t.id === draggedTodo!.id)
    if (todoIndex !== -1) {
      todos.value[todoIndex].status = newStatus
    }
  } catch (err) {
    showError(err instanceof Error ? err.message : 'タスクの更新に失敗しました')
  } finally {
    loading.value = false
    draggedTodo = null
  }
